...
class Cooked_Settings {
	...
	public static function field_radio( $field_name, $options ){
		...
		echo '<p class="cooked-padded">';
			...
			foreach( $options as $value => $name) :
				...
				echo '<input' . $combined_extras . ' type="radio" id="radio-group-' . $field_name . '-' . $value . '" name="cooked_settings[' . $field_name . ']" value="' . $value . '"' . ( isset( $_cooked_settings[$field_name] ) && $_cooked_settings[$field_name] == $value || isset( $_cooked_settings[$field_name][0] ) && $_cooked_settings[$field_name][0] == $value ? ' checked' : '' ) . '/>';
	...
	public static function field_migrate_button( $field_name, $title ){
		...
		if ( $old_recipes != 'complete' ):
			...
			if ( $total > 0 ):
				...
				echo '</p>';
				echo '<p>';
					echo '<span id="cooked-migration-progress" class="cooked-progress"><span class="cooked-progress-bar"></span></span><span id="cooked-migration-progress-text" class="cooked-progress-text">0 / ' . $total . '</span>';
				echo '</p>';
	...
	public static function field_password( $field_name, $placeholder ){
		global $_cooked_settings;
		echo '<p>';
			echo '<input type="password"' . ( $placeholder ? ' placeholder="' . esc_attr( $placeholder ) . '"' : '' ) . ' name="cooked_settings[' . $field_name . ']" value="' . ( isset( $_cooked_settings[$field_name] ) && $_cooked_settings[$field_name] ? $_cooked_settings[$field_name] : '' ) . '">';
		echo '</p>';
	}
